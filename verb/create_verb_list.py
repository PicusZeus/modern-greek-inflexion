...
def create_all_basic_forms(pres_form):
   ...
   :return: a dictionary {'present': '', 'conjunctive': '', 'aorist': '', 'paratatikos': ''} and others, for times it gives active and medio-passive (if exists) divided by '/'. Modals are given in 3rd person, alternative formse are separated by coma
    ...
    if deponens:
        passive_conjugation = recognize_passive_present_continuous_conjugation(pres_form)

        pres_dep_forms = [pres_form]
        ...
        verb_temp['present'] = ','.join(pres_dep_forms)

    elif not_deponens:

        passive_conjugation = recognize_active_non_past_conjugation(pres_form)
        ...
        if pres_pass_forms:
            verb_temp['present'] = pres_form + '/' + pres_pass_forms
        else:
            verb_temp['present'] = pres_form
    ...
    if not_deponens:
        ...
        verb_temp['conjunctive'] = perf_forms
        ...
        if passive_root:
            ...
            verb_temp['conjunctive'] = passive_perf_form
        ...
        verb_temp['conjunctive'] = active_perf_form
        ...
        if perf_root:
            verb_temp['conjunctive'] = passive_root + 'εί'
    ...
    if not_deponens:
        ...
        if passive_root:
            ...
            if 'ποιηθ' in passive_root:
                passive_aor_form = (passive_root + 'ηκα')
            verb_temp['aorist'] = passive_aor_form
    ...
    if not_deponens:
        ...
        if root[-3:] == 'ποι':
            paratatikos = root + 'ούσα/' + root + 'ούμουν'

        verb_temp['paratatikos'] = paratatikos
    ...
    if pres_part_act and pres_part_act in greek_corpus or root[-3:] == 'ποι':
        verb_temp['act_pres_participle'] = pres_part_act
    ...
    if arch_pres_part_act:
        verb_temp['arch_act_pres_participle'] = arch_pres_part_act
    ...
    if pres_part_pass and pres_part_pass in greek_corpus or root[-3:] == 'ποι':
        ...
        verb_temp['pass_pres_participle'] = pres_part_pass
    ...
    if not passive_root and act_root and not passive_perfect_participles:
    ...
    if passive_perfect_participles:
        ...
        verb_temp['passive_perfect_participle'] = ','.join(passive_perfect_participles)
